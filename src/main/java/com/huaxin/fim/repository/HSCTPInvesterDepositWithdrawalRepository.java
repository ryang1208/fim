 ...
 * @version 1.0
 */
@Repository(value = "HSctpInvesterDepositWithdrawalRepository")
@EntityClass(entityClass = HSCTPInvesterDepositWithdrawal.class)
@Forbid(forbiddenMethods = { "deleteAll" })
public interface HSCTPInvesterDepositWithdrawalRepository extends CrudRepository<InvesterDepositWithdrawal, Long> {



	@Modifying
	@Transactional(propagation = Propagation.REQUIRED)
	@Query(value="delete from HSCTPInvesterDepositWithdrawal p "
	...
	@Query("select c from HSCTPInvesterDepositWithdrawal c where c.date=:date and(c.investerID=:investerID) ")
	public List<HSCTPInvesterDepositWithdrawal> findByDateAndInvesterID(@Param("date") String date, @Param("investerID") String investerID);
