 ...
 * @version 1.0
 */
@Repository(value = "HSctpInvesterFundRepository")
@EntityClass(entityClass = HSCTPInvesterFund.class)
@Forbid(forbiddenMethods = { "deleteAll" })
public interface HSCTPInvesterFundRepository extends CrudRepository<HSCTPInvesterFund, Long> {
	...
	@Query(value = "delete from HSCTPInvesterFund i " + "where i.date=:date and (i.investerID=:investerID)")
	...
	@Query("select c from HSCTPInvesterFund c where c.date=:date and(c.investerID=:investerID) ")
	public List<HSCTPInvesterFund> findByDateAndInvesterID(@Param("date") String date, @Param("investerID") String investerID);
