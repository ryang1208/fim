 ...
 * @version 1.0
 */
@Repository(value = "positionDetailsRepository")
@EntityClass(entityClass = PositionDetails.class)
@Forbid(forbiddenMethods = { "deleteAll" })
public interface PositionDetailsRepository extends CrudRepository<PositionDetails, Long> {

	@Query("select pd from PositionDetails pd,ProductInvesterRelationship pr,iWinProduct p where "
			+ "pd.investerID = pr.investerID AND pr.productID = p.id AND pd.brokerID = pr.brokerID "
			+ "AND (p.productName =:productName or :productName is null) "
			+ "AND (pd.date <=:endDate or :endDate is null) "
			+ "AND (pd.date >=:beginDate or :beginDate is null) order by pd.date desc,pd.contact asc")
	...
	public List<PositionDetails> findAll();
