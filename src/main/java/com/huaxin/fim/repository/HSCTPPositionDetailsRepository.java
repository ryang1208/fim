 ...
 * @version 1.0
 */
@Repository(value = "HSctpPositionDetailsRepository")
@EntityClass(entityClass = HSCTPPositionDetails.class)
@Forbid(forbiddenMethods = { "deleteAll" })
public interface HSCTPPositionDetailsRepository extends CrudRepository<HSCTPPositionDetails, Long> {
	...
	public List<HSCTPPositionDetails> findAll();

	@Modifying
	@Transactional(propagation = Propagation.REQUIRED)
	@Query(value="delete from HSCTPPositionDetails p "
	...
	@Query("select c from HSCTPPositionDetails c where c.date=:date and(c.investerID=:investerID) ")
	public List<HSCTPPositionDetails> findByDateAndInvesterID(@Param("date") String date, @Param("investerID") String investerID);
	...
	@Query(value = "SELECT DISTINCT contract.asset_type FROM t_raw_hs_position_details th LEFT JOIN (SELECT DATE,contract,asset_type FROM t_contract WHERE DATE=:date) contract ON "
