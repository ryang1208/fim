...
/**
 * 保存产品指数指标统计数据（信息比率，特雷诺比率，年化跟踪误差指标）
 * 
 * @author liumd
 *
 */
@Named("productIndexPeriodStatisticsDao")
public class ProductIndexPeriodStatisticsDao {

	private static Log logger = LogFactory.getLog(ProductIndexPeriodStatisticsDao.class);
	 ...
	 */
	@Changeable
	public Map<String, Object> saveProductIndexPeriodStatistics(List<ProductIndexPeriodStatistics> list) {
		synchronized(this){
			...
			try {
				// 批量存储
				int count = 0;
				for (ProductIndexPeriodStatistics period_item : list) {
